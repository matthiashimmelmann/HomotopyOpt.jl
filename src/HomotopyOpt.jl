...
=#
function computesystem(p, G::ConstraintVariety, evaluateobjectivefunctiongradient::Function)
...
function resolveSingularity(
    ...
    if length(p)>10
        q = takelocalsteps(
            ...
            Q,
            evaluateobjectivefunctiongradient;
            whichstep = "gaussnewtonstep",
        ).allcomputedpoints[end]
        ...
        if Q(q) < Q(p)
            ...
            for _ in 1:4
                q = gaussnewtonstep_new(G, p + 1e-3*randn(Float64, length(p)), 0, [0.0 for _ in 1:length(p)])
                if Q(q) < Q(p)
                    return (q, true)
                end
            end
...
function gaussnewtonstep(G::ConstraintVariety, p, stepsize, v; tol = 1e-8, initialtime, maxseconds)
    ...
    while (norm(evaluate.(G.fullequations, G.variables=>q)) > tol)
        ...
        if norm(evaluate.(G.fullequations, G.variables=>qnew)) <=
           norm(evaluate.(G.fullequations, G.variables=>q))
...
function gaussnewtonstep_new(G::ConstraintVariety, p, stepsize, v)
    res = newton(System(G.equations; variables = G.variables), p+stepsize*v)
    #display(res)
    return real.(res.x)
end
...
function gaussnewtonstep_HC(G::ConstraintVariety, initial_point, q; max_iters)
    res = newton(G.EDTracker.tracker.homotopy.F.interpreted.system, initial_point, q; max_iters = max_iters)
    if any(isnan.(res.x))
        J = evaluate.(
            differentiate(
...
function EDStep_HC(G::ConstraintVariety, p, stepsize, v; homotopyMethod, amount_Euler_steps = 4)
    ...
    if homotopyMethod=="HomotopyContinuation"
        ...
        global currentSolution, _ =
            gaussnewtonstep_HC(G, currentSolution, q; max_iters = amount_Euler_steps<=0 ? 500 : 10)
        for step in 1:amount_Euler_steps
            q = p+stepsize*((step+1)/(amount_Euler_steps+1))*v
            global currentSolution, _ =
                gaussnewtonstep_HC(G, currentSolution, q; max_iters = amount_Euler_steps==step ? 500 : 10)
        end
...
function onestep(F, p, stepsize)
    solveresult =
        solve(F, [p]; start_parameters = [0.0], target_parameters = [stepsize], show_progress = false)
...
function twostep(F, p, stepsize)
    ...
    solveresult =
        solve(F, [p]; start_parameters = [0.0 + 0.0im], target_parameters = [midparam], show_progress = false)
...
function takelocalsteps(
    ...
    for _ in 1:maxsteps
        ...
        q, Tq, vq1, vq2, success, stepsize = backtracking_linesearch(
            objectiveFunction,
            F,
            G,
            evaluateobjectivefunctiongradient,
...
function findminima(
    ...
    if jacRank==0
        p, optimality = resolveSingularity(
            ps[end],
            G,
            objectiveFunction,
            evaluateobjectivefunctiongradient;
    ...
    while (Base.time() - initialtime) <= maxseconds
        ...
        jacobian = evaluate.(
            differentiate(G.fullequations, G.variables),
            G.variables=>lastLSR.newsuggestedstartpoint,
        )
        ...
        if lastLSR.converged
            ...
            if jR != jacRank || norm(ps[end - 1]-ps[end]) < tolerance^3
                ...
                println("Resolving")
                p, foundsomething = resolveSingularity(
                    lastLSR.allcomputedpoints[end],
                    G,
                    objectiveFunction,
                    evaluateobjectivefunctiongradient;
                    homotopyMethod = homotopyMethod,
                ...
                if foundsomething
                    ...
                    return OptimizationResult(
                        ...
                        p0,
                        lastLSR.newsuggestedstepsize,
                        tolerance,
                        optRes.lastlocalstepsresult.converged,
                        optRes.lastlocalstepsresult,
                        G,
                        evaluateobjectivefunctiongradient,
                        optRes.lastpointisminimum,
                    )
                end
                return OptimizationResult(
                    ps,
...
function watch(
    ...
    if dim == 2
        if !isempty(samples)
            fullx = [
                minimum([q[1] for q in vcat(samples, ps)]) - 0.05,
                maximum([q[1] for q in vcat(samples, ps)]) + 0.05,
            ]
            fully = [
                minimum([q[2] for q in vcat(samples, ps)]) - 0.05,
                maximum([q[2] for q in vcat(samples, ps)]) + 0.05,
            ]
        end
    ...
    elseif dim == 3
        if !isempty(samples)
            ...
            fullz = [
                minimum([q[3] for q in vcat(samples, ps)]) - 0.05,
                maximum([q[3] for q in vcat(samples, ps)]) + 0.05,
            ]
        end
...
function draw(result::OptimizationResult; kwargs...)
    ...
    if dim == 2
        ...
        for q in ps
            plt1 =
                scatter!(plt1, [q[1]], [q[2]], legend = false, color = :black, xlims = fullx, ylims = fully)
        end
        for q in localqs
