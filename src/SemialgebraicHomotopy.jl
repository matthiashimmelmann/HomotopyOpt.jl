...
=#
function resolveSingularity(p, G::SemialgebraicSet, Q::Function, evaluateobjectivefunctiongradient; homotopyMethod=homotopyMethod, traditional_newton=true)
    if Q(q) < Q(p)
        return(q, true) 
    else
        for _ in 1:5
            q = gaussnewtonstep(G, p + 1e-3*randn(Float64, length(p)); traditional_newton=traditional_newton)[1]
            if Q(q) < Q(p)
                return(q, true)
            end
        end
...
=#
function gaussnewtonstep_lagrange(G::SemialgebraicSet, p, reference_point; tol=1e-13, initialtime=Base.time(), maxseconds=100, maxsteps=50)
    F = G.full_EDSystem
    cur_p = vcat(p, [0 for _ in length(p)+1:length(F.variables)])
    ...
    while !isempty(violated_indices)
        ...
        violated_indices = [i for (i,eq) in enumerate(G.inequalities) if evaluate(eq, G.variables=>cur_p)<=-tol]
        new_equations = vcat(G.equalities, G.inequalities[violated_indices])
        new_F = System(new_equations, variables=F.variables, parameters=F.parameters)
    end
	return cur_p, true
...
#=
 We predict in the projected gradient direction and correct by using the Gauss-Newton method
=#
function gaussnewtonstep(G::SemialgebraicSet, p; tol=1e-13, initialtime=Base.time(), maxseconds=100, traditional_newton=true)
	jac = G.fulljacobian[:,1:length(G.equalities)]
    global damping = 0.2
    global q = traditional_newton ? traditional_newton_correct(G.equalities, jac, G.variables, p; tol=tol) : symmetric_newton_correct(G.equalities, jac, G.variables, p; tol=tol)
    ...
    while length(violated_indices) > 0
        global q = traditional_newton ? traditional_newton_correct(new_equations, jac, G.variables, q; tol=tol) : symmetric_newton_correct(new_equations, jac, G.variables; tol=tol)
        violated_indices = [i for (i,eq) in enumerate(G.inequalities) if evaluate(eq, G.variables=>q)<=-tol]
        new_equations = vcat(G.equalities, G.inequalities[violated_indices])
        jac = G.fulljacobian[:,vcat(1:length(G.equalities), violated_indices)]
    end
...
function EDStep_HC(G::SemialgebraicSet, p, stepsize, v; homotopyMethod, amount_Euler_steps=3, traditional_newton=true)
		...
		if all(entry->Base.abs(entry.im)<1e-6, result)
            try
                ...
                for i in 1:amount_Euler_steps
                    setStartSolution(G.EDTracker, vcat(q0, [0. for _ in G.equalities]))
                    start_parameters!(G.EDTracker.tracker, q0)
...
=#
function isMinimum(G::SemialgebraicSet, Q::Function, evaluateobjectivefunctiongradient, p::Vector; tol=1e-4, criticaltol=1e-3, traditional_newton=true)
	H = hessian(Q, p)
	...
	# Else take a small step in gradient descent direction and see if the energy decreases
		...
		q = gaussnewtonstep(G, p - 1e-2 * evaluateobjectivefunctiongradient(p); initialtime=Base.time(), maxseconds=10, traditional_newton=true)[1]
		return Q(q)<Q(p)
	end
end
...
=#
function backtracking_linesearch_momentum(Q::Function, G::SemialgebraicSet, evaluateobjectivefunctiongradient::Function, p0::Vector, stepsize::Float64; whichstep="EDStep", maxstepsize=5, initialtime, maxseconds, homotopyMethod="HomotopyContinuation", r=1e-4, s=0.9, traditional_newton=true)
	_, _, basegradient = get_NTv(p0, G, evaluateobjectivefunctiongradient)
	α = [stepsize]
	p = Base.copy(p0)
    while length(α) <= 7 && time()-initialtime <= maxseconds
        try
		    global q, success = stepchoice(G, whichstep, α[end], p0, basegradient; traditional_newton=traditional_newton=traditional_newton)
        catch e
            @warn e
            maxstepsize = α[end]/2
            push!(α, α[end]/2)
...
=#
function backtracking_linesearch(Q::Function, G::SemialgebraicSet, evaluateobjectivefunctiongradient::Function, p0::Vector, stepsize::Float64; whichstep="EDStep", maxstepsize=5, initialtime, maxseconds, homotopyMethod="HomotopyContinuation", r=1e-4, s=0.9, traditional_newton=true)
	_, _, basegradient = get_NTv(p0, G, evaluateobjectivefunctiongradient)
	α = [0, stepsize]
	p = Base.copy(p0)
    while true
		...
		if ( ( Q(q) > Q(p0) - r*α[end]*basegradient'*basegradient || (Q(q) > Q(p0) && q!=p0) ) && success)
			helper = zoom(α[end-1], α[end], Q, evaluateobjectivefunctiongradient, G, whichstep, p0, basegradient, r, s; initialtime, maxseconds, homotopyMethod, traditional_newton)
		...
		if basegradient'*vq <= 0 && success
			helper = zoom(α[end], α[end-1], Q, evaluateobjectivefunctiongradient, G, whichstep, p0, basegradient, r, s; initialtime, maxseconds, homotopyMethod, traditional_newton)
			_, Tq, vq = get_NTv(helper[1], G, evaluateobjectivefunctiongradient)
			return helper[1], Tq, vq, helper[2], helper[end]
		end
		if (success)
...
=#
function zoom(αlo, αhi, Q, evaluateobjectivefunctiongradient, G, whichstep, p0, basegradient, r, s; initialtime, maxseconds, homotopyMethod, traditional_newton)
    ...
    while index <= 7
        ...
        try
            global q, success = stepchoice(G, whichstep, α, p0, basegradient; traditional_newton=traditional_newton)
        catch e
            @warn e
...
function get_NTv(q, G::SemialgebraicSet,
                    evaluateobjectivefunctiongradient::Function; tol=1e-8)
    active_indices = [i for (i,eq) in enumerate(G.fullequations) if i>length(G.equalities) && abs(evaluate(eq, G.variables=>q)) < tol]#
    ...
    if length(G.equalities)==0 && length(active_indices)==0
        Nq_active, Tq_active = [0. for _ in 1:length(G.variables)], nullspace(zeros(Float64,length(G.variables),length(G.variables)))
    else
...
function takelocalsteps(p::Vector{Float64}, ε0::Float64, tolerance, G::SemialgebraicSet,
                ...
                maxsteps=6, maxstepsize=4, decreasefactor=2.5, initialtime = Base.time(), maxseconds = 100, whichstep="EDStep", homotopyMethod="HomotopyContinuation", momentum_strategy=true, tangent_predictor=[], θ_collect=[1], traditional_newton=true)
    _, Tp, vp = get_NTv(p, G, evaluateobjectivefunctiongradient)
    ...
    global tangent_predictor, stepsize_collect = isempty(tangent_predictor) ? Base.copy(p) : tangent_predictor, [stepsize]
    for _ in 1:maxsteps
        if !momentum_strategy
            global q, Tq, vq, success, stepsize = backtracking_linesearch(objectiveFunction, G, evaluateobjectivefunctiongradient, q_collect[end], Float64(stepsize); whichstep, maxstepsize, initialtime, maxseconds, homotopyMethod, traditional_newton=traditional_newton)
            ...
            _q, success = stepchoice(G, whichstep, 1, q_collect[end], y-q_collect[end]; traditional_newton=traditional_newton) # Momentum Step
            if !success
                push!(θ_collect, θ_collect[end]/decreasefactor)
                continue
            end
            global q, Tq, vq, success, stepsize = backtracking_linesearch_momentum(objectiveFunction, G, evaluateobjectivefunctiongradient, _q, Float64(stepsize); whichstep, maxstepsize, initialtime, maxseconds, homotopyMethod, traditional_newton=traditional_newton)
        ...
        push!(q_collect, q); push!(T_collect, Tq); push!(n_collect, norm(vq, Inf)); push!(v_collect, vq)
		length(T_collect)>3 ? deleteat!(T_collect, 1) : nothing
        if n_collect[end] < tolerance
            return q_collect[2:end],v_collect[2:end],n_collect[2:end],q,stepsize,true,timesturned,θ_collect,tangent_predictor
        end
        ...
        stepsize = Base.minimum([ Base.maximum([ success ? stepsize*((v_collect[end-1]'*v_collect[end-1])*(evaluateobjectivefunctiongradient(q_collect[end-1])'*evaluateobjectivefunctiongradient(q_collect[end-1])))^(1/4)/((v_collect[end]'*v_collect[end])*(evaluateobjectivefunctiongradient(q_collect[end])'*evaluateobjectivefunctiongradient(q_collect[end])))^(1/4)  : 0.1*stepsize, 1e-4]), maxstepsize])
    end
    return q_collect[2:end],v_collect[2:end],n_collect[2:end],q_collect[end],stepsize,false,timesturned,θ_collect,tangent_predictor
end
...
function minimize(p0::Vector{Float64}, tolerance::Float64,
                G::SemialgebraicSet,
                objectiveFunction::Function;
                maxseconds=100, maxlocalsteps=5, initialstepsize=0.2, whichstep="EDStep", initialtime = Base.time(), homotopyMethod = "HomotopyContinuation", momentum_strategy=true, momentum_factor=0.9, traditional_newton=true)
    ...
    while (Base.time() - initialtime) <= maxseconds
        ...
        global computedpoints,_,_,q,suggestedstepsize,converged,_,θ_collect,tangent_predictor = takelocalsteps(p, ε0, tolerance, G, objectiveFunction, evaluateobjectivefunctiongradient; maxsteps=maxlocalsteps, initialtime=initialtime, maxseconds=maxseconds, whichstep=whichstep, homotopyMethod=homotopyMethod, momentum_strategy=momentum_strategy, tangent_predictor=tangent_predictor, θ_collect=θ_collect, traditional_newton=traditional_newton)
        ...
        if converged
			...
			if norm(ps[end-1]-ps[end]) < tolerance^3
				optimality = isMinimum(G, objectiveFunction, evaluateobjectivefunctiongradient, ps[end]; traditional_newton=traditional_newton, criticaltol=tolerance)
				if optimality
					return OptimizationResult(true,ps,initialstepsize,tolerance,true,G,objectiveFunction,optimality)
				end
				println("Resolving")
				_q, foundsomething = resolveSingularity(q, G, objectiveFunction, evaluateobjectivefunctiongradient; homotopyMethod=homotopyMethod, traditional_newton=traditional_newton)
				if foundsomething
	...
	return OptimizationResult(true,ps,p0,ε0,tolerance,converged,G,evaluateobjectivefunctiongradient,optimality)
end
